...
namespace CSharpCourse.Linq.Ex01
{
    ...
    public class Solution
    {
        ...
        private static void DisplayInfoAboutShows(List<Show> allShows)
        {
            ...
            foreach (var title in allShows.Select(s => s.Title))
            {
            ...
            allShows.Where(show => show.StartAt.Hours >= 21).ToList().ForEach(show =>
                WriteInfoAboutShow(show)
            );
            ...
            foreach (var channel in allShows.Select(x => x.Channel).Distinct())
            {
